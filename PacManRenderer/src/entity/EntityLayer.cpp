...
EntityLayer::EntityLayer(std::unique_ptr<std::vector<std::unique_ptr<entity::IEntityStateRenderer>>> p_render_entities,
                         float scale,
...
void EntityLayer::Initialise() {
  for (const std::unique_ptr<entity::IEntityStateRenderer>& p_render_entity : *(this->_p_render_entities))
